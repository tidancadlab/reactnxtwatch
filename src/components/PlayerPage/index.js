...
import {FailView, Loading} from '../Components'
import './index.css'
...
class Trending extends Component {
  ...
  onGetVideos = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.setState({
          videos: videoList(data.video_details),
  ...
  onSuccess = ({
    ...
    const isSaved =
      savedVideoList.filter(value => value.id === videos.id).length > 0
      ...
      return (
        <Container className="player" gap="16px">
          ...
          <Container align="stretch" gap="16px" className="data_container">
            ...
            <Container
              className="view_container sm_view"
              row
            ...
            >
              <Container row style={{fontSize: 14}} className="self_start">
                ...
                <BsDot color="#64748b" />
                <Text color="#64748b">
                  {formatDistanceToNow(new Date(videos.publishedAt))}
                </Text>
              </Container>
              <ItemContainer>
                <Item>
          ...
          <Container
            justify="flex-start"
            className="profile_video data_container"
            row
            gap="16px"
          >
            <Container>
              <Img
                width="64px"
                src={videos.channel.profileImageUrl}
                alt="channel logo"
              />
            </Container>
