...
import {FailView, Loading, Banner} from '../Components'
...
class Home extends Component {
  ...
  onGetVideos = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.setState({
          fetchStatus: statusCode.success,
          videos: videos.map(value => videoList(value)),
        })
      } else {
  ...
  onSuccess = ({theme}) => {
    ...
    if (fetchStatus === statusCode.failed) {
      return <FailView onGetVideos={this.onGetVideos} />
    }
    ...
    return (
      <Store.Consumer>
        {value => (
          <Container
            className="page"
            bg={!value.theme ? '#f9f9f9' : '#181818'}
            data-testid="home"
          >
            {value.isBannerShow && (
              <Banner onBannerClose={value.onBannerClose} />
            )}
            <Container className="page_container">
              <SearchField borderColor={value.theme ? '#909090' : '#cccccc'}>
                <SearchInput
                  type="search"
                  color={value.theme ? 'white' : 'black'}
              ...
              {fetchStatus === statusCode.pending ? (
                <Loading />
              ) : (
